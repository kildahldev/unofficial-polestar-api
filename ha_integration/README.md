...
Copy `custom_components/polestar/` into your HA `config/custom_components/` directory (or install via HACS as a custom repo). The `unofficial-polestar-api` dependency is installed automatically.

Restart HA, then add the integration via **Settings → Devices & Services → Add Integration → Polestar**. Enter your Polestar ID email, password, and VIN (Vehicle Identification Number). Each config entry sets up one vehicle — add the integration again with a different VIN if you have multiple cars. Tick **Demo mode** to get a fake vehicle with static data (no API connection needed).
...
Requires [mushroom-cards](https://github.com/piitaya/lovelace-mushroom) and [card-mod](https://github.com/thomasloven/lovelace-card-mod) via HACS. Shows battery level, range, charging status, lock state, and plug status with a battery progress bar. Tapping navigates to the full vehicle dashboard.
...
Add a map card below for live GPS tracking:
...
This integration is unofficial and not affiliated with Polestar. Use at your own risk.
